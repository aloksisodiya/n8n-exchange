...
function StatCard({ label, value, color, icon }) {
  return (
    <div style={{
      ...
      <div style={{ color, fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-display)' }}>{value}</div>
      <div style={{ color: 'var(--text-faint)', fontSize: 11, marginTop: 4 }}>{label}</div>
    </div>
  )
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            <h3 style={{ color: 'var(--text-primary)', fontSize: 15, fontFamily: 'var(--font-display)', fontWeight: 700 }}>
              {workflow.name}
            </h3>
          </div>
      ...
      <div style={{ display: 'flex', gap: 20 }}>
        {[
          { label: 'NODES',      val: workflow.nodes?.length || 0, color: 'var(--accent-blue)' },
          { label: 'EDGES',      val: workflow.edges?.length || 0, color: 'var(--accent-purple)' },
          { label: 'EXECUTIONS', val: workflow.executions || 0,    color: 'var(--accent-yellow)' },
        ].map(s => (
          <div key={s.label}>
        ...
        {workflow.lastRun && (
          <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
            ...
            <div style={{ color: 'var(--text-faint)', fontSize: 10 }}>{formatDate(workflow.lastRun)}</div>
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h2 style={{ color: 'var(--text-secondary)', fontSize: 13, fontWeight: 700, letterSpacing: 1 }}>
          ...
          {workflows.length === 0 ? (
            <div style={{
              ...
              No workflows yet.{' '}
              <Link to="/builder/new" style={{ color: 'var(--accent-yellow)' }}>Create your first one →</Link>
            </div>
          ) : (
        ...
        <div style={{
          ...
          <h3 style={{ color: 'var(--text-secondary)', fontSize: 12, fontWeight: 700, letterSpacing: 1, marginBottom: 16 }}>
            QUICK START — COMMON WORKFLOWS
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
            {[
              { name: 'DCA Bot',          desc: 'Timer → Spot Buy',          icon: '⏱', color: '#f59e0b' },
              { name: 'Stop Loss Guard',  desc: 'Price Below → Spot Sell',   icon: '🛑', color: '#f97316' },
              { name: 'Take Profit',      desc: 'Price Above → Spot Sell',   icon: '🎯', color: '#06b6d4' },
              { name: 'Leverage Long',    desc: 'Price Below → Long',        icon: '⬆', color: '#22c55e' },
              { name: 'Hedge Strategy',   desc: 'Price Below → Long + Short',icon: '◎', color: '#818cf8' },
              { name: 'Alert Bot',        desc: 'Timer → Telegram',          icon: '📨', color: '#38bdf8' },
            ].map(template => (
