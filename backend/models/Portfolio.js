...
portfolioSchema.methods.addHolding = function (symbol, name, quantity, price) {
  ...
  if (existingHolding) {
    ...
    existingHolding.profitLossPercentage =
      (existingHolding.profitLoss / totalInvested) * 100;
...
portfolioSchema.methods.removeHolding = function (
  symbol,
  quantity,
  currentPrice,
) {
  ...
  const proportionalInvested =
    (quantity / holding.quantity) * holding.totalInvested;
